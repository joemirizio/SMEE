...
public class Main extends SimpleApplication implements ActionListener, SceneProcessor {
	...
	private Kinect kinect1;
	private Kinect kinect2;
	
	//public static final Vector3f KINECT_1_DEFAULT_LOC = new Vector3f(0.3f, -0.75f, 0.55f);
	//public static final Vector3f KINECT_2_DEFAULT_LOC = new Vector3f(-0.3f, -0.75f, 0.55f);
	public static final Vector3f KINECT_1_DEFAULT_LOC = new Vector3f(0.4f, -0.7f, 0.55f);
	public static final Vector3f KINECT_2_DEFAULT_LOC = new Vector3f(-0.4f, -0.7f, 0.55f);
    ...
    public void simpleInitApp() {
		...
		Geometry prediction_line = new Geometry("AlertZone", prediction_line_cylinder);
		...
		// Kinect Sensors
		kinect1 = new Kinect("Kinect1");
		kinect1.setLocalTranslation(new Vector3f(0, 6, 0));
		kinect1.lookAt(KINECT_1_DEFAULT_LOC);
		rootNode.attachChild(kinect1.getMainNode());


		kinect2 = new Kinect("Kinect2");
		kinect2.setLocalTranslation(new Vector3f(0, 6, 0));		
		kinect2.lookAt(KINECT_2_DEFAULT_LOC);
		rootNode.attachChild(kinect2.getMainNode());

		// Kinect Viewports
		kinect1.getCamera().setViewPort(0f, 0.625f, 0f, 0.625f);
		ViewPort kinect_vp2 = renderManager.createMainView("Kinect1", kinect1.getCamera());
		kinect_vp2.setClearFlags(true, true, true);
		kinect_vp2.attachScene(rootNode);
		kinect_vp2.setBackgroundColor(ColorRGBA.Pink);
		
		kinect2.getCamera().setViewPort(1.0f, 1.625f, 0f, 0.625f);
		ViewPort kinect_vp1 = renderManager.createMainView("Kinect2", kinect2.getCamera());
		kinect_vp1.setClearFlags(true, true, true);
		kinect_vp1.attachScene(rootNode);
		kinect_vp1.setBackgroundColor(ColorRGBA.Pink);
		
		//bb = BufferUtils.createByteBuffer(Kinect.RES_X * Kinect.RES_Y * 4);
		//bi = new BufferedImage(Kinect.RES_X, Kinect.RES_Y, BufferedImage.TYPE_4BYTE_ABGR);
    }

    @Override
    public void simpleUpdate(float tpf) {	
		/*if (test_child != null) {
			if (kinect1.getCamera().containsGui(test_child.getWorldBound())) {
	...
	private Spatial createTestChild(TestChild.Type test_child_type) {
		/*Geometry child = new Geometry("Child-" + test_child_type, new Box(0.5f, 0.05f, 0.5f));
		Material unshaded_mat = new Material(assetManager, "Common/MatDefs/Misc/Unshaded.j3md");
		unshaded_mat.setColor("Color", ColorRGBA.Brown);
		child.setMaterial(unshaded_mat);*/
	...
	public void onAction(String name, boolean is_pressed, float tpf) {
		...
		if (name.equals("LEFT_CLICK")) {
			kinect1.lookAt(cam.getDirection());
		} else if (name.equals("RIGHT_CLICK")) {
			kinect2.lookAt(cam.getDirection());
		}
		...
		/*if (name.equals("SPACE1") && is_pressed) {
			...
			System.out.println("Kinect 1: " + kinect1.getCamera().getDirection());
			System.out.println("Kinect 2: " + kinect2.getCamera().getDirection());

			System.out.println("Hit space!");
			
			int width = Kinect.RES_X;
			int height = Kinect.RES_Y;
			
			//ViewPort vp = renderManager.getMainViews().get(0);
			ViewPort vp = renderManager.getMainView("Kinect1Process");
			ViewPort kinect2_vp = renderManager.getMainView("Kinect2");
			if (vp == null) {
				System.out.println("New viewport");
				
				vp = renderManager.createMainView("Kinect1Process", kinect1.getCamera());
